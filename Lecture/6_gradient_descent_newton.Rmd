...
**BFGS**: The idea of BFGS method is to update $B_k$ using $B_{k+1}=B_k+\alpha {u} {u}^{T}+\beta {v} {v}^{T}$. Imposing the secant condition, $B_{k+1} {s}_k={y}_k$. Choosing ${u}={y}_k$ and ${v}=B_k {s}_k$, we can solve for $\alpha, \beta$:
\begin{align*}
