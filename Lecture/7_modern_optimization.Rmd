...
In the neural networkwe setting, $w$ refers to both the weights and biases. 
...
Note that above is the **vanilla gradient descent**, aka, **batch gradient descent**. As we need to calculate the gradients for the whole dataset to perform just one update. 


# Gradient Descent with Momentum
One major problem with gradient descent is that the update of the parameters is slow, because gradients in flat regions are very small.

The gradient descent with momentum instead update the velocity using the gradient vector, then update the parameters (locations) using the velocity.
...
Note that, if $\gamma=0$. we recover the gradient descent. When $\gamma\neq 0$, setting $v_0=0$, then one can see that
\begin{align*}
{v}_{t}=\gamma \text {v}_{t-1}+\alpha \nabla L(w_{t})=\gamma^{t-1}  \alpha \nabla L(w_{1})+\gamma^{t-2} \alpha \nabla L(w_{2})+\ldots+\gamma \alpha \nabla L(w_{t-1})+\alpha \nabla L(w_{t})
\end{align*}
therefore, the current velocity depends on the histories of the gradients. 

With Momentum update, the parameter vector will build up velocity in any direction that has consistent gradient. It mains an exponentially decaying average of gradients. Note that 
$$v_t=\alpha \sum_{j=1}^t \gamma^{t-j} \nabla L(w_j),$$ 
therefore the weight for $\nabla L(w_j)$ is $\alpha \gamma^{t-j}$, a weight factor that decays exponentially as further in the past (i.e., $j$ decreases). So $\gamma$ is the exponential decay rate. The larger $\gamma$, the slower is the decay.
...
An equivalent way to implement is:
...
of the valley. We do something to reduce these oscillations. 

We know that we will use our momentum term $\gamma v_{t-1}$ to
move the parameters $w_t$. Computing $w_t-\gamma v_{t-1}$ thus gives us an approximation
of the next position of the parameters (the gradient is missing for the full update), a rough idea where our parameters are going to be. We can now
...
Looking ahead helps correcting its course quicker than momentum
...
\begin{align*} 
w_{\text{look ahead},t} &=w_{t} + \gamma  {v}_{t-1} \\ 
{v}_{t} &=\gamma \cdot {v}_{t-1} - \alpha \nabla L(w_{\text{look ahead},t}) \\ 
w_{t+1} &=w_{t} + v_{t} 
\end{align*}
...
Consider $L(w)=E_{\mathcal{P}_n}[\ell(Y, f(X; w))]$, where $\mathcal{P}_n$ is the empirical distribution of $(X, Y)$'s, the population version of $L(w)$ is $E[\ell(Y, f(X; w))]$, in principle, for the descent algorithm, any gradient $g$ such that $E(g)=E[\nabla\ell(Y, f(X; w))]$ would work. Obviously, $\nabla L(w)$ is one example. Yet $\nabla L(w)$ goes over the entire
data once before updating the parameters. The pros is that the gradient is accurately calculated. The cons is that the data size is large, it is very slow to update parameters. 

More generally, in each step, instead of 
$$
w_{t+1} = w_t - \alpha \nabla L(w_t),
$$
$\nabla L(w_t)$ may be replaced with the gradient at one point at a time as we update 
$$
...
However, the drawback is for each update the gradient is not accurately estimated. A compromise is to do the mini-batch gradient descent by using gradient
$$\frac{1}{|B_t|}\sum_{i\in B_t} \ell(y_i, f(x_i; w_t)),$$
...
- Total number of steps (iterations): the total number of epochs times number of batches $(n / B)$.
...
The commonly used mini-batch sizes are betwelen 50 and 256 . This method is typically the choice when training a neural network with a large data set. The general rule is

- For a relatively small training data set, with $\leq 2000$ observations, use batch gradient descent.
...
The choice of the learning rate is delicate and very influential on the convergence of the SGD algorithm.

The algorithm update the parameters after it sees $B$ data points. 
```python
X, Y: data matrices
...
- $\mathrm{B}=$ Mini batch size 

\begin{align*}
\begin{array}{cc}
\text { Algorithm } & \# \text { of steps in } 1 \text{epoch } \\
\hline \text { Vanilla (Batch) Gradient Descent } & 1 \\
...
- Exponential decay - as the name signifies, it is represented as $\epsilon_t=\epsilon_0 e^{-k t}$ where $\epsilon_0$ and $k$ are hyperparameters and $t$ is the iteration number or epoch number.
- Polynomial decay - this is represented as $\epsilon_t=\frac{\epsilon_0}{(1+k t)^a}$ where, $\epsilon_0$ and $a, k$ are the hyperparameters and $t$ is the iteration number. This is often used in stochastic gradient descent (i.e., batch size=1).
...
**Adagrad**:
  ...
  Note momentum and Nesteroc momentum update all components of $w$ with the *same learning rate* $\alpha$.
Adagrad, or the **adaptive subgradient method**, adapst a learning rate for each component of $w$. A component adaptive discount factor is in place to adjust learning rate. This factor is a square root of the cumulative sum of squared gradients.
...
Adagrad excels when the gradient is sparse. It dulls the inflennce of parameters with consistently high gradients, thereby increasing the influence of parameters with infrequent updates. One weakness of Adagrad is that components of $s$ are each strictly non-decreasing. Therefore, after a while, frequent parameters will have infinitesimally small effective learning rate before convergence.
...
RMSProp (root mean square propgation) extends Adagrad to prevent a rapid monotonically decreasing learning rate. RMSprop divides the learning rate by an
*exponentially decaying moving average* of squared gradients.
...
KM (2022): p.295. For adaptive learning rates methods, the base learning rate is still needed. It is shown that with the weighted exponentially decaying moving average methods, they may result in non-convergence even in convex problems.
