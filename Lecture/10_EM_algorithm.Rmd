...
\end{align*}
One can also show that $E_{h}\Big[\log\frac{h(z)}{g(z)}\Big] =0$ if and only if $h=g$.
...
Suppose $X_i\overset{iid}{\sim}\frac{1}{4}N(\mu_1,1)+\frac{3}{4}N(\mu_2,1)$, $\theta=(\mu_1,\mu_2).$  
...
If $W_{1i}\sim f_1$, $W_{2i}\sim f_2$, and $Z_i\perp(W_{1i},W_{2i})$, $\{Z_i,W_{1i},W_{2i}\}_{i=1}^n$ are independent, we can write
$$
...
- Let $Z_{(R)}=\sum_r Z_{(r)} I(R=r)$. Similarly, $Z_{\bar{R}}=\sum_{r} Z_{(r)} I(R \neq r)$. 
- The ideal full data is $(R, Z)$. The observed data is $\left(R, Z_{(R)}\right)$. 
...
The likelihood based on $\left.\left(R_i=r_i, z_{\left(R_i\right)i}=z_{\left(r_i\right)}\right)\right)_{i=1}^n$ is given by
...
\end{align*}
The secon equality holds because
\begin{align*}
...
\end{align*}
here the expectation is conditional on $(R_i, Z_{(r)i})$ with parameter $\theta'$. The last equality holds because of MAR.
...
\end{align*}
where ${E}_{\theta^{t-1}}[\mathbf{S}({\mu})]:= \left({E}_{\theta^{t-1}}\left[Z_i Z_i^{\top}\right]+{\mu} {\mu}^{\top}-2 {\mu} {E}_{\theta^{t-1}}\left[Z_i\right]^{\top}\right).$ The key is to compute ${E}_{\theta^{t-1}}\left[Z_i\right]$ and ${E}_{\theta^{t-1}}\left[Z_i Z_i^{\top}\right]$ (expected sufficient statistics) and they can be computed using conditional formula for multivariate Gaussian distribution. The solution in the M-step then is given by the MLE solution with the expected sufficient statistics:
\begin{align*}
