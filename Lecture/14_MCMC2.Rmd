...
\begin{equation*}
H(x) = -J \sum_{i \sim j} x_i x_j - h\sum_{i} x_i,  \text{ where }x \in \mathcal{X}
\end{equation*}
...
Using Bayes' rule, 
\begin{align*}
p_{X \mid Y}(x \mid y)=\frac{p_{Y \mid X}(y \mid x) p_X(x)}{p_Y(y)}=\frac{1}{Z_y} \exp \left(-H_y(x)\right)
\end{align*}
where 
\begin{align*}
H_y(x)=-\frac{1}{\sigma^2} \sum_{i \in I} y_i x_i+H(x)=-\frac{1}{\sigma^2} \sum_{i \in I} y_i x_i+\beta \sum_{\substack{i \sim j}} x_i x_j.
\end{align*}
