...
Now $\hat x=(A^TA)^{-1}A^Tb$ if $A^TA$ is invertible, and then $\hat b = A(A^TA)^{-1}A^Tb$ (even if it exists).

Note: $A^TA \hat x=A^Tb$ is called Normal Equation.

**Fact 1:** Normal equation always has at least one solution.

**Fact 2:** $A^TA$ is invertible if and only if $A$ has full-column rank.

**Fact 3:** The solution to the normal equation is not necessary a solution to the original problem $Ax=b$.
