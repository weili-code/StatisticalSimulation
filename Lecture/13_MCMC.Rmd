...
\text{One-step transition density function}:\ &f(x_0, v):= \frac{\partial F(v|x_0)}{\partial v}= P(X^{(n+1)}=v|X^{(n)}=x_0)\\
\end{align*}
...
$P(X^{(n+m)} \in A| X^{(n)}=x_n)= \int_A f^{[m]}(x_n,x_{n+m})dx_{n+m}.$ By similar argument, we have the **Chapman-Kolmogorov equation**:
$$
...
where $g(w)$ is viewed as the start distribution and $f(w,v)$ is the transition density function.
