...
(1)
$f(x)=a^\top x :{\mathbb{R^n}}\to{\mathbb{R}}$, $f(x)$ can also written in $f(x)={\sum_{i=1}^{n}}a_ix_i$,
the gradient for $f(x)$,
$${\nabla}f(x)=a.$$
(2)
$f(x)=x^\top Ax$ ($A$ is square matrix), the gradient for $f(x)$,
$$
{\nabla}f(x)=Ax+A^\top x;
$$
...
$$
\begin{aligned}
f(x)&=\lVert Ax-b\rVert^2\\&=\langle Ax-b, Ax-b\rangle\\&=(Ax-b)^\top(Ax-b)
