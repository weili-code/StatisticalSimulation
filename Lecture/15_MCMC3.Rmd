...
Assume for *each* move $m\in M$, (1) there is a set $C_m \subset \mathbb{R}^d \times \mathbb{R}^d$ such that
$$
...
Suppose we like to move between two spaces $\mathcal{X}_1 \subset \mathbb{R}^{n_1}$ and $\mathcal{X}_2 \subset \mathbb{R}^{n_2}$, $n_1 \neq n_2$, where the whole space is $\mathcal{X}=\mathcal{X}_1 \cup \mathcal{X}_2.$ Let $M=\{1, 2\}$ denote the model type or space type. 
...
- generate some $w\in \mathbb{R}^{d_1} \sim g_{1\to2}$ according to some density function $g_{1\to2}$
- use some deterministic bijective mapping:
$$
h_{1\to2}: (x, w) \mapsto (x' ,w') \quad \mathbb{R}^{n_1+d_1} \to 
\mathbb{R}^{n_2+d_2}
$$
which is *dimensions matching* ($n_1+ d_1= n_2+ d_2$) to obtain $x' \in \mathcal{X}_2$.
- the map $h_{1\to2}$ is assumed to be a diffeomophism (continuously differentiable and so is its inverse).
- when moving backward from $x'\in \mathcal{X}_2$ to $x \in \mathcal{X}_1$, we sample some $w' \sim g_{2\to1}$, and move under the deterministic bijective mapping $h_{2\to1}:=h_{1\to2}^{-1}$ where
$$
h_{2 \to 1}: (x', w') \mapsto (x, w) \quad \mathbb{R}^{n_2+d_2} \to 
\mathbb{R}^{n_1+d_2}
$$
That is
\begin{align*}
...
h_{1\to2}: \mathbb{R}^{n_1+d_1} \longrightarrow \mathbb{R}^{n_2+d_2} & h_{2\to1}: \mathbb{R}^{n_2+d_2} \longrightarrow \mathbb{R}^{n_1+d_1}
\end{array}
\end{align*}
- The acceptance probability is given by 
\begin{align*}
\alpha_{1\to 2}\left(x' \mid x\right)=\min \left\{1, \frac{\pi\left(x'\right) g_{2 \rightarrow 1}\left(w'\right)}{\pi\left(x\right) g_{1 \rightarrow 2}\left(w \right)}\left|\frac{\partial {h}_{1 \rightarrow 2}\left(x, w \right)}{\partial\left(x, w\right)}\right|\right\},
\end{align*}
...
(i) It may be that either $d_1=0$ or $d_2=0$ as long as the dimensions are matched $n_1+d_1=n_2+d_2.$
...
With starting value $k^{(0)} \in K$, $X^{(0)} \in \mathcal{X}_{k^{(0)}}$. For $s=0, 1, 2, \cdots$:

1. Choose $k^{*}$ with probability $j_{k^{(s)} \to k^{*}}(k^{(s)},X^{(s)})$. If $k^{*}=k^{(s)}$, perform the usual MH update, that is $X^{*} \sim q_{k^{(s)}}(\cdot|k^{(s)}, X^{(s)})$ (some usual proposal transition density in the space $\mathcal{X}_{k^{(s)}}$), and compute 
...
2. If $k^{*} \neq k^{(s)}$, we continue by generating some $W^{(s)} \sim g_{k^{(s)} \to k^{*}}(\cdot|k^{(s)}, X^{(s)})$.
3. Set $(X^{*}, W^{*})= h_{k^{(s)}\to k^{*}}(X^{(s)}, W^{(s)})$
4. Compute
\begin{align*}
\alpha\left(X^* \mid {X}^{(s)}\right)=\min \left\{1, \frac{j_{k^{*}\to k^{(s)}}(k^{*}, X^*)  \pi\left(k^{*}, {X}^{*}\right) g_{k^{*} \to k^{(s)}}\left(W^{(*)}|k^{*}, X^{*}\right)}{j_{k^{(s)}\to k^{*}}(k^{(s)}, {X}^{(s)})  \pi\left(k^{(s)}, X^{(s)}\right) g_{k^{(s)} \to k^{*}}\left(W^{(s)}|k^{(s)}, X^{(s)}\right)}\left|\frac{\partial {h}_{k^{(s)}\to k^{*}}\left(X^{(s)}, W^{(s)}\right)}{\partial\left(X^{(s)}, W^{(s)}\right)}\right|\right\},
\end{align*}
5. Generate $U \sim \mathrm{U}(0,1)$, if $U \leq \alpha(X^*|X^{(s)})$, set $k^{(s+1)} \leftarrow k^{*}$, $X^{(s+1)} \leftarrow X^*$; otherwise set $k^{(s+1)} \leftarrow k^{(s)}$, $X^{(s+1)} \leftarrow X^{(s)}$. 
...
(6) Here we allow the proposal density $g_{k^{(s)} \to k^{*}}(\cdot)$ dependent on the current state, i.e., $g_{k^{(s)} \to k^{*}}(\cdot|k^{(s)}, X^{(s)})$, for simpler design model, this needs not be so.
(7) This is possible to extend the above algorithm by allowing drawing a different move type $m\in M$ and all subsequent functions $j, q, h$ and $g$ may be made dependent on $m$, with acceptance probability adjusted by a ratio of the type move probabilities.
