...
With starting value $m^{(0)} \in M$, $X^{(0)} \in \mathcal{X}_{m^{(0)}}$. For $s=0, 1, 2, \cdots$:

1. Choose $m^{*}$ with probability $j_{m^{(s)} \to m^{*}}(X^{(s)})$. If $m^{*}=m^{(s)}$, perform the usual MH update, that is $X^{*} \sim q_{m^{(s)}}(\cdot|X^{(s)})$ (some usual proposal transition density for the type $m^{(s)}$), and compute 
$$\alpha(X^*|X^{(s)}) = \min \left(1, \frac{q_{m^{(s)}}(X^{(s)}|X^*)\pi(X^*)}{q_{m^{(s)}}(X^*|X^{(s)})\pi(X^{(s)})} \right);$$
then skip to the step 5.
2. If $m^{*} \neq m^{(s)}$, we continue by generating some $W^{(s)} \sim g_{m^{(s)} \to m^{*}}$.
3. Set $(X^{*}, W^{*})= h_{m^{(s)}\to m^{*}}(X^{(s)}, W^{(s)})$
4. Compute
\begin{align*}
\alpha\left(X^* \mid {X}^{(s)}\right)=\min \left\{1, \frac{j_{m^{*}\to m^{(s)}}(X^*)  \pi\left({X}^{*}\right) g_{m^{*} \to m^{(s)}}\left(W^{(*)}\right)}{j_{m^{(s)}\to m^{*}}({X}^{(s)})  \pi\left(X^{(s)}\right) g_{m^{(s)} \to m^{*}}\left(W^{(s)}\right)}\left|\frac{\partial {h}_{m^{(s)}\to m^{*}}\left(X^{(s)}, W^{(s)}\right)}{\partial\left(X^{(s)}, W^{(s)}\right)}\right|\right\},
\end{align*}
5. Generate $U \sim \mathrm{U}(0,1)$, if $U \leq \alpha(X^*|X^{(s)})$, set $m^{(s+1)} \leftarrow m^{*}$, $X^{(s+1)} \leftarrow X^*$; otherwise set $m^{(s+1)} \leftarrow m^{(s)}$, $X^{(s+1)} \leftarrow X^{(s)}$. 

**Remark**:

(1) It requires that $\sum_{m'}j_{m \to m'}=1$. It also requires that if $j_{m \to m'}=0$, then $j_{m' \to m}=0$--otherwise the acceptance probability is not well defined.
(2) For any $m, m'$ such that $j_{m \to m'} \neq 0$, it requires that $h_{m \to m'}$ to be a diffeomorphism.
(3) It is allowed that either $W^*$ or $W^{(s)}$ is zero dimensional (null), as long as the dimensions matching is maintained.
