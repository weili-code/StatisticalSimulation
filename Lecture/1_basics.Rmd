...
**Kernel space (null space)**: The kernel space of an $m \times n$ matrix $A$, written as $\rm{ker}(A)$, is the set of all solutions of the homogeneous equation $Ax=0$. In set notation,
\begin{align*}
...
**Determinant of a matrix**: Determinant is a number associated with any square matrix; we’ll write
...
**Transpose of a matrix**: The transpose of a matrix $A=[a_{i,j}]$ is given by $A^T=[a_{j,i}].$

**Inner prodcut**: Suppose $x, y \in \mbb{R}^n$, Euclidean inner product is $\langle x, y\rangle= x^Ty=\sum_{i=1}^n x_iy_i.$ The Euclidean norm of $x$ is $\|x\|:=\sqrt{\langle x, x\rangle}=\sqrt{\sum_{i=1}^nx_i^2}.$
...
Column space and Kernel space: $\operatorname{Col}(A)^{\perp}=\operatorname{Ker}\left(A^T\right)$ and $\operatorname{Row}(A)^{\perp}=\operatorname{Ker}(A)$
...
An $m \times n$ matrix $U$ has orthonormal columns if $U^TU=I$.

An $m \times m$ square matrix $P$ is called an orthogonal matrix if
\begin{align*}
P P^{T}=P^{T} P=I_{m}, \quad \text { or } P^{-1}=P^{T} .
\end{align*}
...
**Positive definite matrix**: A square and symmetric matrix $A$ is called a positive definite matrix (denoted by $A>0$), if $x^TAx>0$ for all $x\neq 0$. Similarly, one can define negative definite matrix. Both positive definite matrices and negative definite matrices are invertible. But the converse is not true. 

**Positive semi-definite matrix**: A square and symmetric matrix $A$ is called a positive semi-definite matrix (denoted by $A\geq 0$), if $x^TAx\geq 0$ for all $x\neq 0$. For a positive semi-definite matrix, it is invertible if and only if it is also positive definite. Similarly, one can define negative semi-definite matrix. 
...
$$
P_xy=\frac{\inner{x}{y}}{\inner{x}{x}}x=\frac{xx'}{\inner{x}{x}}y
$$
